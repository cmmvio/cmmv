...
class DefaultHTTPTranspiler extends core_1.AbstractTranspile {
    run() {
...
export class ${serviceName}Generated extends AbstractService {
    ...
    ${contract.services
        ...
        const outputFilePath = path.join(outputDirGenerated, serviceFileNameGenerated);
        ...
        if (generateBoilerplates) {
            ...
            const outputFileGenerated = path.join(outputDirGenerated, serviceFileNameGenerated);
...
${contract.services
        ...
        throw new Error("Function ${service.functionName} not implemented");
            ...
            if (!fs.existsSync(outputFilePathFinal)) {
                fs.writeFileSync(outputFilePathFinal, this.removeExtraSpaces(serviceTemplate), 'utf8');
            }
        }
        else {
...
${contract.services
    ...
    getControllerDecorators({ authRouter, rootOnlyRouter = false, hasCache, contract }, medatada, authRole = 'get') {
        let decoracotrs = '';
    ...
    generateController(contract) {
        ...
        const hasOpenAPIModule = core_1.Module.hasModule('openapi');
        ...
        if (hasCache)
            extraImports.push(`import { Cache, CacheService } from "@cmmv/cache";`);
        if (authRouter)
            extraImports.push(`import { Auth } from "@cmmv/auth";`);
        if (hasOpenAPIModule) {
            extraImports.push(`import { ApiTags } from "@cmmv/openapi";`);
            if (contract.options.tags)
                openAPIControllerDecorators.push(`@ApiTags(${typeof contract.options.tags === 'string' ? '"' + contract.options.tags + '"' : '"' + contract.options.tags.join(',') + '"'})`);
        }
...
${contract.services
    ...
    async ${service.functionName}(@Body() payload: ${service.request}, @Req() req): Promise<${service.response}> {
        return this.${serviceName.toLowerCase()}.${service.functionName}(payload);
            ...
            .join('\n\n')}}`;
        if (!telemetry) {
            controllerTemplateGenerated = this.removeTelemetry(controllerTemplateGenerated);
        }
        const outputGeneratedDir = this.getGeneratedPath(contract, 'controllers');
        const outputFilePath = path.join(outputGeneratedDir, controllerFileNameGenerated);
        fs.writeFileSync(outputFilePath, this.removeExtraSpaces(controllerTemplateGenerated), 'utf8');
...
export class ${controllerName} extends ${controllerName}Generated {}`;
        if (generateBoilerplates) {
            ...
            const outputFileGenerated = path.join(outputGeneratedDir, controllerFileName);
            const outputFilePathFinal = path.join(outputDir, controllerFileName);
            if (!fs.existsSync(outputFilePathFinal)) {
                fs.writeFileSync(outputFilePathFinal, this.removeExtraSpaces(controllerTemplate), 'utf8');
            }
        }
        else {
...
@Controller('${controllerPath}')
export class ${controllerName} extends ${controllerName}Generated {}`), 'utf8');
        }
    }
    ...
    importServices(importsFromModel, contract) {
