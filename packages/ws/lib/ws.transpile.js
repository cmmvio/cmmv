...
class WSTranspile extends core_1.AbstractTranspile {
    run() {
        const contracts = core_1.Scope.getArray('__contracts');
        const generateGateways = core_1.Config.get('rpc.generateGateways', true);
...
export class ${gatewayName}Generated {
    ...
    async insert(@Data() data: Add${contract.controllerName}Request, @Socket() socket){
        try{
            ...
            const response = await RpcUtils.pack(
                ...
                { item: result, id: ${core_1.Config.get('repository.type') === 'mongodb' ? `result._id` : `result.id`} }
            );

            ${hasCache ? `\n            CacheService.set(\`${cacheKeyPrefix}\${${core_1.Config.get('repository.type') === 'mongodb' ? `result._id` : `result.id`}}\`, JSON.stringify(result), ${cacheTtl});` : ''}
    ...
    async delete(@Data() data: Delete${contract.controllerName}Request, @Socket() socket){
        ...
        const outputFilePath = path.join(outputDirGenerated, gatewayFileNameGenerated);
        fs.writeFileSync(outputFilePath, this.removeExtraSpaces(gatewayTemplateGenerated), 'utf8');
        //Service
...
${contract.services
        ...
        throw new Error("Function ${service.functionName} not implemented");
            ...
            const outputFileGenerated = path.join(outputDirGenerated, gatewayFileName);
            const outputFilePathFinal = path.join(outputDir, gatewayFileName);
            if (!fs.existsSync(outputFilePathFinal)) {
                fs.writeFileSync(outputFilePathFinal, this.removeExtraSpaces(gatewayTemplate), 'utf8');
            }
        }
        else {
