...
const core_2 = require("@cmmv/core");
const archiver = require('archiver');
let BackupService = class BackupService {
    constructor() {
        this.logger = new core_2.Logger('BackupService');
        this.backupsDir = path.join(__dirname.replace('src', 'backups'));
    ...
    async createBackup() {
        ...
        try {
            ...
            for (const entity of entities) {
                try {
                    const result = await this.backupEntity(entity, currentBackupDir);
                    stats.entitiesProcessed++;
                    stats.totalRecords += result.recordCount;
                    stats.files.push(result.fileName);
                    this.logger.log(`Backup da entidade ${entity.name} concluído: ${result.recordCount} registros`);
                }
                catch (error) {
                    this.logger.error(`Erro ao fazer backup da entidade ${entity.name}: ${error.message}`);
                    stats.errors.push({
                        entity: entity.name,
                        error: error.message,
                    });
            ...
            const metaFilePath = path.join(this.backupsDir, `${zipFileName}.meta.json`);
            ...
            this.logger.error(`Erro crítico no processo de backup: ${error.message}`);
            this.cleanupTempDir(currentBackupDir);
            throw error;
        }
    }
     ...
     */
    getAllEntities() {
        const instance = repository_1.Repository.getInstance();
        const result = [];
     ...
     * @returns {Promise<any>}
     */
    async backupEntity(entityInfo, backupDir) {
        const { name, target, tableName } = entityInfo;
        const fileName = name.replace(/Entity$/, '').toLowerCase() + '.json';
        const filePath = path.join(backupDir, fileName);
        const repository = repository_1.Repository.getRepository(target);
        const allRecords = [];
        let totalRecords = 0;
        let currentPage = 0;
        const pageSize = 1000;
        ...
        while (hasMoreRecords) {
            ...
            const queryResult = await repository_1.Repository.findAll(target, {
                limit: pageSize,
                offset: offset,
            });
            if (queryResult &&
                ...
                currentPage++;
                this.logger.log(`Entidade ${name}: processados ${totalRecords} registros...`);
            }
            else {
    ...
    async listBackups() {
        ...
        return metaFiles
            ...
            try {
                ...
                this.logger.error(`Erro ao ler metadados do backup ${file}: ${error.message}`);
                return null;
            }
        })
     ...
     * @returns {Promise<any>}
     */
    async downloadBackupFile(filename, res) {
        ...
        try {
            ...
            fileStream.on('error', (err) => {
                this.logger.error(`Error streaming backup file: ${err.message}`);
                try {
                    if (!res.sent) {
                        res.code(500).json({
                            success: false,
                    ...
                    this.logger.error(`Could not send error response: ${e.message}`);
                }
            });
            return await new Promise((resolve, reject) => {
                ...
                fileStream.on('error', (err) => {
                    this.logger.error(`Error during download stream: ${err.message}`);
                    reject(err);
                });
            });
        }
        catch (error) {
            this.logger.error(`Error preparing backup download: ${error.message}`);
