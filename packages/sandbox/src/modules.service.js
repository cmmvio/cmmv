...
let ModulesService = class ModulesService {
    constructor() {
        this.logger = new core_1.Logger('ModulesService');
        ...
        this.availableModules = [
            {
                name: 'AI',
                installed: false,
                description: 'Artificial Intelligence integration with multiple providers',
            ...
            },
            {
                name: 'Elastic',
                installed: false,
                description: 'Elasticsearch integration for advanced search capabilities',
                category: 'Integration',
     ...
     */
    updateInstalledStatus(packageJson) {
        ...
        for (const module of this.availableModules) {
            ...
            const mainPackage = module.dependencies?.find((dep) => dep.startsWith('@cmmv/') &&
            ...
            if (module.submodules) {
                for (const submodule of module.submodules) {
                    const submodulePackage = submodule.packageName ||
     ...
     */
    async checkForUpdates(modules) {
        const modulesToCheck = modules.filter((m) => m.installed && m.version);
        for (const module of modulesToCheck) {
            try {
                const mainPackage = module.dependencies?.find((dep) => dep.startsWith('@cmmv/') &&
                    dep.toLowerCase() ===
                        `@cmmv/${module.name.toLowerCase()}`) || module.dependencies?.[0];
                if (!mainPackage)
                    continue;
                ...
                if (latestVersion) {
                    ...
                    if (installedVersion && latestVersion) {
                        try {
                            ...
                            this.logger.log(`Error comparing versions for ${module.name}: ${err.message}`);
                ...
                this.logger.log(`Error checking update for ${module.name}: ${error.message}`);
            }
        }
    }
    /**
    ...
    async getLatestVersion(packageName) {
        try {
            const response = await axios_1.default.get(`https://registry.npmjs.org/${encodeURIComponent(packageName)}/latest`, { timeout: 5000 });
            if (response.data && response.data.version)
                return response.data.version;
            return null;
        }
        catch (error) {
            this.logger.log(`Error getting version for ${packageName}: ${error.message}`);
            return null;
        }
    }
    /**
    ...
    async getAllModules() {
        ...
        this.checkForUpdates(this.availableModules).catch((err) => this.logger.error(`Error checking updates: ${err.message}`));
        return this.availableModules;
    }
    checkModuleEnabled(mainTs, module) {
    ...
    async getModuleDetails(moduleName) {
        const allModules = await this.getAllModules();
        const module = allModules.find((m) => m.name.toLowerCase() === moduleName.toLowerCase());
        return module || null;
    }
    /**
     ...
     */
    buildInstallCommand(packageManager, packages) {
     ...
     */
    async installModule(moduleName, dependencies) {
        try {
            this.logger.log(`Installing module ${moduleName}...`);
            if (!dependencies || dependencies.length === 0)
                throw new Error(`No dependencies specified for module ${moduleName}`);
            const { spawn } = require('child_process');
            const packageManager = this.detectPackageManager();
            const installCommand = this.buildInstallCommand(packageManager, dependencies);
            ...
            return new Promise((resolve, reject) => {
                ...
                installProcess.on('close', (code) => {
                    if (code === 0) {
                        this.logger.log(`Module ${moduleName} installed successfully`);
                        resolve({ moduleName, success: true, output });
                    }
                    else {
                        this.logger.error(`Failed to install module ${moduleName} with code ${code}`);
            ...
            this.logger.error(`Error installing module ${moduleName}: ${error.message}`);
            throw error;
        }
    }
    /**
     ...
     * @returns The success and message of the operation
     */
    async installSubmodule(moduleName, submoduleName, packageName) {
        try {
            this.logger.log(`Installing submodule ${submoduleName} for module ${moduleName}...`);
            if (!packageName)
                throw new Error(`No package name specified for submodule ${submoduleName}`);
            const { spawn } = require('child_process');
            const packageManager = this.detectPackageManager();
            ...
            return new Promise((resolve, reject) => {
                ...
                installProcess.on('close', (code) => {
                    if (code === 0) {
                        this.logger.log(`Submodule ${submoduleName} installed successfully`);
                        resolve({
                            moduleName,
                            submoduleName,
                            success: true,
                        ...
                        this.logger.error(`Failed to install submodule ${submoduleName} with code ${code}`);
                        reject(new Error(`Installation failed with code ${code}: ${output}`));
                    }
                });
            });
            ...
            this.logger.error(`Error installing submodule ${submoduleName}: ${error.message}`);
            throw error;
        }
    }
    /**
     ...
     * @returns The success and message of the operation
     */
    async updateModule(moduleName, dependencies) {
        try {
            ...
            this.logger.log(`Using package manager: ${packageManager} for update`);
            ...
            return new Promise((resolve, reject) => {
                ...
                updateProcess.on('close', (code) => {
                    if (code === 0) {
                        this.logger.log(`Module ${moduleName} updated successfully`);
                        resolve({ moduleName, success: true, output });
                    }
                    else {
                        this.logger.error(`Failed to update module ${moduleName} with code ${code}`);
                        reject(new Error(`Update failed with code ${code}: ${output}`));
                    }
                });
            });
        }
        catch (error) {
            this.logger.error(`Error updating module ${moduleName}: ${error.message}`);
            throw error;
        }
    }
    /**
    ...
    async isModuleEnabled(moduleName) {
        try {
            ...
            let allProvidersPresent = true;
            if (moduleImport.providers?.length) {
                const providersMatch = mainTs.match(/providers:\s*\[([\s\S]*?)\]/);
                if (providersMatch) {
     ...
     */
    async toggleModule(moduleName, enable) {
        try {
            let mainTs = this.readMainTs();
            const module = this.availableModules.find((m) => m.name === moduleName);
            if (!module?.moduleImport) {
                throw new Error(`No module import configuration found for ${moduleName}`);
            }
            const { moduleImport } = module;
            if (enable) {
                // Adicionar imports
    ...
    insertImport(content, importStatement) {
    ...
    addModuleToArray(content, moduleName) {
    ...
    removeModuleFromArray(content, moduleName) {
        return content
    ...
    addProviderToArray(content, provider) {
    ...
    removeProviderFromArray(content, provider) {
        return content
...
exports.ModulesService = ModulesService = tslib_1.__decorate([
    (0, core_1.Service)('modules')
], ModulesService);
