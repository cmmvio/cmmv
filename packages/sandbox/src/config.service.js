...
let ConfigService = class ConfigService {
    constructor() {
        this.logger = new core_1.Logger('ConfigService');
        this.configFilePath = path.join(process.cwd(), '.cmmv.config.cjs');
        this.packageModulesPath = path.join(process.cwd(), 'node_modules/@cmmv');
        this.monorepoModulesPath = path.join(process.cwd(), 'packages');
    }
    /**
    ...
    async readConfig() {
        try {
            if (!fs.existsSync(this.configFilePath)) {
                this.logger.log(`Config file not found at ${this.configFilePath}`);
                return {};
            }
     ...
     */
    stringifyConfig(config, indent = 2) {
    ...
    async getModuleConfigs() {
        ...
        const monorepoModules = await this.findModuleConfigsInDir(this.monorepoModulesPath);
        ...
        const nodeModules = await this.findModuleConfigsInDir(this.packageModulesPath, '@cmmv');
        for (const module of nodeModules) {
            if (moduleConfigsMap.has(module.name)) {
                ...
                const mergedOptions = this.mergeOptions(existing.options, module.options);
    ...
    mergeOptions(options1, options2) {
        ...
        const mergeOption = (target, source) => {
            const merged = { ...target };
            ...
            if (source.group)
                merged.group = source.group;
            if (source.env)
                merged.env = source.env;
            // Mesclar arrays
            if (source.enum) {
                merged.enum = Array.from(new Set([...(target.enum || []), ...source.enum]));
            }
                    ...
                    for (const [key, sourceProp] of Object.entries(source.properties)) {
                        if (merged.properties[key]) {
                            ...
                            merged.properties[key] = {
                                ...
                                default: sourceProp.default !== undefined
                                    ? sourceProp.default
    ...
    async findModuleConfigsInDir(directory, prefix = '') {
        const moduleConfigsMap = new Map();
        ...
        try {
            ...
            for (const packageJsonPath of packageJsonPaths) {
                try {
                    ...
                    const moduleName = packageJson.name?.replace('@cmmv/', '') ||
                    ...
                    for (const configFile of configFiles) {
                        if (fs.existsSync(configFile)) {
                            try {
                                ...
                                const configKeys = Object.keys(configModule).filter((key) => key.endsWith('Config'));
                                for (const configKey of configKeys) {
                                    const moduleSchema = configModule[configKey];
                                    const fileOptions = [];
                                    // Processar cada seção do schema
                                    for (const [sectionKey, sectionSchema,] of Object.entries(moduleSchema)) {
                                        this.processSchemaSection(sectionKey, sectionSchema, fileOptions, currentConfig[moduleName]);
                                    }
                                    ...
                                    if (moduleOptions.length === 0) {
                                        ...
                                        moduleOptions = this.mergeOptions(moduleOptions, fileOptions);
                                    }
                                }
                            }
                            catch (error) {
                                this.logger.error(`Error loading config from ${configFile}: ${error.message}`);
                            }
                        }
                    }
                    ...
                    if (moduleOptions.length > 0) {
                        if (moduleConfigsMap.has(moduleName)) {
                            ...
                            moduleConfigsMap.set(moduleName, {
                                ...existing,
                                options: this.mergeOptions(existing.options, moduleOptions),
                            });
                        }
                        else {
                    ...
                    this.logger.error(`Error processing module at ${packageJsonPath}: ${error.message}`);
                }
            }
            return Array.from(moduleConfigsMap.values());
        }
        catch (error) {
            this.logger.error(`Error searching for module configs in ${directory}: ${error.message}`);
            return [];
        }
    }
    processSchemaSection(sectionKey, schema, options, currentConfig, parentKey = '') {
        for (const [key, definition] of Object.entries(schema)) {
            ...
            if (definition.type === 'object' && definition.properties) {
                ...
                const option = {
                    ...
                    type: 'object',
                    description: definition.description || `${key} configuration`,
                    required: definition.required || false,
                    group: sectionKey,
                    properties: {},
                };
                // Processar sub-propriedades
                for (const [propKey, propDef] of Object.entries(definition.properties)) {
            ...
            for (const corePath of possibleCorePaths) {
                ...
                if (configFile) {
                    ...
                    if (coreConfig) {
                        return {
                            name: 'core',
                            description: coreConfig.description || 'Core Configuration',
     ...
     */
    async getConfigValue(moduleName, key) {
     ...
     */
    async setConfigValue(moduleName, key, value) {
        try {
            const config = await this.readConfig();
     ...
     */
    async updateModuleConfig(moduleName, moduleConfig) {
        try {
            const config = await this.readConfig();
