...
vi.mock('@cmmv/http', () => ({
    ...
    Get: vi.fn(() => (target: any, propertyKey: string, descriptor: PropertyDescriptor) => descriptor),
    Param: vi.fn(() => (target: any, propertyKey: string, parameterIndex: number) => {}),
}));
...
describe('RepositoryController', () => {
    ...
    describe('handlerListDatabase', () => {
        it('should call Repository.listDatabases', async () => {
            ...
            vi.mocked(Repository.listDatabases).mockResolvedValue(mockDatabases);
        ...
        it('should throw error when Repository.listDatabases fails', async () => {
            vi.mocked(Repository.listDatabases).mockRejectedValue(new Error('Database connection failed'));

            await expect(controller.handlerListDatabase()).rejects.toThrow('Database connection failed');
        });

        it('should return multiple databases', async () => {
            const mockDatabases = { databases: ['main', 'test', 'analytics', 'backup'] };
            vi.mocked(Repository.listDatabases).mockResolvedValue(mockDatabases);
    ...
    describe('handlerListTables', () => {
        ...
        it('should throw error when database does not exist', async () => {
            vi.mocked(Repository.listTables).mockRejectedValue(new Error('Database not found'));

            await expect(controller.handlerListTables('nonExistent')).rejects.toThrow('Database not found');
        });
        ...
        it('should handle special characters in database name', async () => {
            ...
            const result = await controller.handlerListTables('my-database_123');

            expect(Repository.listTables).toHaveBeenCalledWith('my-database_123');
            expect(result).toEqual(mockTables);
        });

        it('should return multiple tables', async () => {
            const mockTables = { tables: ['users', 'products', 'orders', 'categories', 'reviews'] };
            vi.mocked(Repository.listTables).mockResolvedValue(mockTables);
    ...
    describe('error handling', () => {
        it('should propagate errors from listDatabases', async () => {
            ...
            await expect(controller.handlerListDatabase()).rejects.toThrow('Connection timeout');
        });
        ...
        it('should propagate errors from listTables', async () => {
            ...
            await expect(controller.handlerListTables('restrictedDb')).rejects.toThrow('Permission denied');
        });

        it('should handle database access errors', async () => {
            vi.mocked(Repository.listTables).mockRejectedValue(new Error('Access denied for user'));

            await expect(controller.handlerListTables('securedDb')).rejects.toThrow('Access denied for user');
        });

        it('should handle network errors', async () => {
            vi.mocked(Repository.listDatabases).mockRejectedValue(new Error('Network unreachable'));

            await expect(controller.handlerListDatabase()).rejects.toThrow('Network unreachable');
        });
    });
});
